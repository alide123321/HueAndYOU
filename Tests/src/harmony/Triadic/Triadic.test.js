...
describe('Triadic Harmony Strategy', () => {
  ...
  test('buildPalette returns a Palette instance with colors', () => {
    ...
    expect(palette.colorMap.size).toBeGreaterThanOrEqual(4);

    // make sure the expected roles exist (if includeBgTextColors is true)
...
test('triadic hues are ~120° and ~240° from base (tolerant)', () => {
  ...
  // Map preserves insertion order: [primary, secondary, accent, support]
